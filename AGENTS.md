...
- Repo: `https://github.com/rose/rose`
...
- Plugins cross into core only via `rose/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- Legacy config repair belongs in `rose doctor --fix`, not startup/load-time core migrations. Runtime paths use canonical contracts.
...
- CLI: `pnpm rose ...` or `pnpm dev`; build: `pnpm build`.
...
- Use `$rose-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Use `$rose-pr-maintainer` immediately for maintainer-side ROSE issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. Contributor PR creation/refresh follows the requested contributor workflow; linked refs alone do not require maintainer archive tooling.
...
- Naming: **ROSE** product/docs; `rose` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `rose/plugin-sdk/*`.
...
- Use `$rose-docs` for docs writing/review. Docs change with behavior/API.
...
- Docs final answers: include relevant full `https://docs.rose.ai/...` URL(s). If issue/PR work too, GitHub URL last.
...
- Contributor-facing changelog entries thank credited human `@author`. Never thank bots, `@rose`, `@clawsweeper`, or `@steipete`; if unknown, omit thanks.
...
- Secrets: channel/provider creds in `~/.rose/credentials/`; model auth profiles in `~/.rose/agents/<agentId>/agent/auth-profiles.json`.
- Dependency patches/overrides/vendor changes need explicit approval. `pnpm-workspace.yaml` patched dependencies use exact versions only.
- Carbon pins owner-only: do not change `@buape/carbon` unless Shadow (`@thewilloftheshadow`, verified by `gh`) asks.
- Releases/publish/version bumps need explicit approval. Use `$rose-release-maintainer`.
- GHSA/advisories: `$rose-ghsa-maintainer` / `$security-triage`. Secret scanning: `$rose-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `rose gateway restart/status --deep`; logs = `./scripts/clawlog.sh`. No launchd/ad-hoc tmux.
...
- Version bump surfaces live in `$rose-release-maintainer`.
- Parallels: `$rose-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `rose doctor`.
